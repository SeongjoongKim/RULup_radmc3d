...
# Grid parameters for Cylindrical ----------------------------------
nr  = 300       # Radial grid（int）
nz = 100    # Elevation grid (z-direction, int)
ntheta = 1        # Azimuthal grid 1 (y-direction, int)
rin = 1e-1 * au   # inner radius
rout = 300 * au # outer radius(cm)
#zmid = 1.0e-2 * au
#zsrf = 1.0e1 * au
...
# Make the coordinates  ----------------------------------
ri       = np.logspace(np.log10(rin),np.log10(rout),nr+1)      # [cm]
zi       = np.linspace(0,thetaup,nz+1)           # [rad]
thetai   = np.linspace(0.e0,np.pi*2.e0,ntheta+1)               # [rad]
rc       = 0.5 * ( ri[0:nr] + ri[1:nr+1] )
zc       = 0.5 * ( zi[0:nz] + zi[1:nz+1] )
thetac   = 0.5 * ( thetai[0:ntheta] + thetai[1:ntheta+1] )
# Make the grid ----------------------------------
qq       = np.meshgrid(rc,zc,thetac,indexing='ij')
rr       = qq[0]    # cm
tt       = qq[1]     # rad  Spherical theta
zz       = np.tan(tt)*rr  # cm
...
r0 = 10.*au
t0 = ( (10**0.16*ls)/(16*np.pi*r0**2*SB) )**0.25    # 30*(10**0.16)**0.25 # Adopting the DSHARP values
pltt = -0.5
tdust = t0 * (rc/r0)**pltt   # 2D T_mid distribution in (r,theta)
cs       = np.sqrt(kb*tdust/(2.3*mp))   # [cm/s]Isothermal sound speed
omk      = np.sqrt(GG*mstar/rc**3)      # [s^-1] The Kepler angular frequency
...
for i in range(0,nr):
    for j in range(0,nz):
        if (zz[i,j,0]<=z0[i]):
            rhog[i,j,0] = ( sigmag[i,j,0] / (np.sqrt(2.e0*np.pi)*hp[i]) ) * np.exp(-zz[i,j,0]**2/hp[i]**2/2.e0)  #[g/cm3]
            Bp[i,j,0] = B_mid[i,j,0]
            vz[i,j,0] = Cw*sigmag[i,j,0]*omk[i]/rhog[i,j,0]
        else:
            R0 = z0[i]-zz[i,j,0]+rr[i,j,0]
            ...
            rhog[i,j,0] = rhog_R0 * np.exp(-8.0) * np.exp(-(cs_R0/R0/omk_R0)**(-0.6) * np.sqrt((zz[i,j,0]-(4*hp_R0))/R0 ) )
            Bp[i,j,0] = B_mid_R0* (rr[i,j,0]/R0)**alpha_B *np.cos(45.*np.pi/180.)  # Bai 2016, ApJ, 818,152
            vz[i,j,0] = Cw*sigmag[i,j,0]*omk[i]*Bp[i,j,0]/B_mid_R0/rhog[i,j,0] *np.cos(45.*np.pi/180.)
...
vphi = np.sqrt(GG*mstar/np.sqrt(rr**2+zz*2))
...
plt.pcolormesh(np.log10(rr[:,:,0]/au),np.log10(zz[:,:,0]/rr[:,:,0]),rhog[:,:,0],vmin=1e-16,vmax=1e-9,norm=colors.LogNorm(vmin=1e-16,vmax=1e-9),cmap='jet',shading='gouraud',rasterized=True)
...
plt.plot(np.log10(rr[:,0,0]/au),np.log10(4*hp/rr[:,0,0]),'k',lw=2,label=r'4 Hp')
#plt.plot(np.log10(rr[:,0,0]/au),np.log10(zb/rr[:,0,0]),'k--',lw=2,label=r'Zb')
plt.xlim(-1,2.4)
plt.ylim(-2,-0.2)
plt.xlabel(r'log$_{10}$(R$_{cyl}$/1 AU)', fontsize=15)
plt.ylabel(r'log$_{10}$(z/R$_{cyl}$)',fontsize=15)
plt.legend(prop={'size':15},loc=0)
plt.tick_params(which='both',length=6,width=1.5)
plt.colorbar() #ticks=[1e-15,1e-14,1e-13,1e-12])
plt.savefig('gas_dens_cylindrical_rz.pdf',dpi=100, bbox_inches='tight')
plt.clf()


plt.figure(figsize=(10,6))
plt.pcolormesh(np.log10(rr[:,:,0]/au),np.log10(zz[:,:,0]/rr[:,:,0]),vz[:,:,0]*1e-5,vmin=1e-2,vmax=1e0,norm=colors.LogNorm(vmin=1e-2,vmax=1e0),cmap='jet',shading='gouraud',rasterized=True)
#plt.pcolormesh(np.log10(rr/au),np.log10(zzr),rhod_biggr,cmap='gist_heat',norm=colors.LogNorm(vmin=3e-13,vmax=3e-10),shading='gouraud')
plt.plot(np.log10(rr[:,0,0]/au),np.log10(4*hp/rr[:,0,0]),'k',lw=2,label=r'4 Hp')
#plt.plot(np.log10(rr[:,0,0]/au),np.log10(zb/rr[:,0,0]),'k--',lw=2,label=r'Zb')
plt.xlim(-1,2.4)
...
plt.savefig('Vz_cylindrical_rz.pdf',dpi=100, bbox_inches='tight')
plt.clf()


plt.figure(figsize=(10,6))
plt.pcolormesh(np.log10(rr[:,:,0]/au),np.log10(zz[:,:,0]/rr[:,:,0]),Bp[:,:,0],vmin=1e-3,vmax=1e0,norm=colors.LogNorm(vmin=1e-3,vmax=1e0),cmap='jet',shading='gouraud',rasterized=True)
#plt.pcolormesh(np.log10(rr/au),np.log10(zzr),rhod_biggr,cmap='gist_heat',norm=colors.LogNorm(vmin=3e-13,vmax=3e-10),shading='gouraud')
plt.plot(np.log10(rr[:,0,0]/au),np.log10(4*hp/rr[:,0,0]),'k',lw=2,label=r'4 Hp')
#plt.plot(np.log10(rr[:,0,0]/au),np.log10(zb/rr[:,0,0]),'k--',lw=2,label=r'Zb')
plt.xlim(-1,2.4)
...
plt.savefig('Bz_cylindrical_rz.pdf',dpi=100, bbox_inches='tight')
plt.clf()
