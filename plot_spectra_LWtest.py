    ...
    for i in range(nx):
        for j in range(ny):
            if mask[j,i]:
                ...
                try:
                    [ a[k], b[k], c[k], aa[k],  d[k] ], pcov1 = curve_fit(Double, vax, data[:,j,i], p0=[0.01,vmodel[j,i]*1e-3,0.2,0.01,0], bounds=([1e-3,-5.0,0,1e-3,0] ,[0.5,5.0,2.0,0.5,1.0] ), absolute_sigma=True)
                except RuntimeError:
                    print("Error - curve_fit failed")
                k += 1
...
parser.add_argument('-PAmin', default=0, type=int, help='The lower PA in degree unit. Default is 0')
...
dirName = '../spec_figure/Line_width_test/'+mole+'_r{:4.2f}-{:4.2f}_PA{:03d}-{:03d}_'.format(r_min,r_max,PA_min,PA_max)+bmaj+'_LWtest/'
if double == 'T':
    dirName = '../spec_figure/Line_width_test/'+mole+'_r{:4.2f}-{:4.2f}_PA{:03d}-{:03d}_'.format(r_min,r_max,PA_min,PA_max)+bmaj+'_LWtest_double/'
...
turbname = 'RULup_'+mole+'_fiducial_wind_Thermal+Turb_'+bmaj+'.fits'#
cube_turb = imagecube(fdir2+turbname)
...
if double == 'F':
    at,bt,ct,dt,idxt,idyt = Spec_fit(cube_turb.data,cube_turb.velax,rvalst,tvalst,vmodt,r_min,r_max,PA_min,PA_max)
    with open(dirName+'Spec_fit_rmin{:4.2f}_rmax{:4.2f}_params_Turb.dat'.format(r_min,r_max),'w+') as f:
        f.write('Averages of a b c d\n')
        f.write('%13.6e %13.6e %13.6e %13.6e\n'%(at.mean(),bt.mean(),ct.mean(),dt.mean()))
        f.write('a b c d idx idy\n')                   # Include r,theta, phi in coordinates
        for i in range(len(a)):
            f.write('%13.6e %13.6e %13.6e %13.6e %4d %4d\n'%(at[i],bt[i],ct[i],dt[i],idxt[i],idyt[i]))
if double == 'T':
    at,bt,ct,at2,dt,idxt,idyt = Spec_double(cube_turb.data,cube_turb.velax,rvalst,tvalst,vmodt,r_min,r_max,PA_min,PA_max) #bw2,cw2,
    with open(dirName+'Spec_fit_rmin{:4.2f}_rmax{:4.2f}_params_Turb_double.dat'.format(r_min,r_max),'w+') as f:
        f.write('Averages of a0 b0 c0 a1 d0\n') #b1 c1
        f.write('%13.6e %13.6e %13.6e %13.6e %13.6e \n'%(at.mean(),bt.mean(),ct.mean(),at2.mean(),dt.mean())) #%13.6e %13.6e # ,bw2.mean(),cw2.mean()
        f.write('a0 b0 c0 a1 d0 idx idy\n')                   # Include r,theta, phi in coordinates   b1 c1
        for i in range(len(a)):
            f.write('%13.6e %13.6e %13.6e %13.6e %13.6e %4d %4d\n'%(at[i],bt[i],ct[i],at2[i],dt[i],idxt[i],idyt[i])) # %13.6e %13.6e # bw2[i],cw2[i],


# =======================================================================================
# Thermal + Wind
# =======================================================================================
fdir3 = '/Users/kimsj/Documents/RADMC-3D/radmc3d-2.0/RU_Lup_test/Automatics/Fin_script/fiducial_wind/'
windname = 'RULup_'+mole+'_fiducial_wind_Thermal+wind_'+bmaj+'.fits'#
cube_wind = imagecube(fdir3+windname)
#for ii in range(cube_wind.header['NAXIS3']): cube_wind.data[ii,:,:] = cube_wind.data[ii,:,:].T
#cube_wind.velax *= cube.header['CDELT3']/cube_wind.header['CDELT3']
...
# =======================================================================================
fdir = '/Users/kimsj/Documents/RADMC-3D/radmc3d-2.0/RU_Lup_test/Automatics/Fin_script/fiducial_wind/'
fiduname = 'RULup_'+mole+'_fiducial_wind_'+bmaj+'.fits'  #
cubefidu = imagecube(fdir+fitsname)
#for ii in range(cube.header['NAXIS3']): cube.data[ii,:,:] = cube.data[ii,:,:].T
rvalsf, tvalsf, _ = cubefidu.disk_coords(x0=dxc,y0=dyc,inc=inc,PA=DPA,z0=z0,psi=psi,z1=z1,phi=phi)
vmodf = cubefidu._keplerian(rpnts=rvalsf,mstar=0.63,dist=160.0, inc=inc)
vmodf *= np.cos(tvalsf)
b_maj = cubefidu.header['BMAJ']*3.6e3; b_min = cubefidu.header['BMIN']*3.6e3; bpa = cubefidu.header['BPA']
pixsize_x = abs(cubefidu.header['CDELT1']*3.6e3); pixsize_y = abs(cubefidu.header['CDELT2']*3.6e3)
gaussian_2D_kernel = Gaussian2DKernel(b_maj/pixsize_x/np.sqrt(8*np.log(2)),b_min/pixsize_y/np.sqrt(8*np.log(2)),bpa/180.*np.pi,x_size=151, y_size=151)
# Convolve the image through the 2D beam
vmodf = convolve(vmodf,gaussian_2D_kernel,boundary='fill',fill_value='0',normalize_kernel=True)
if double == 'F':
    af,bf,cf,df,idxf,idyf = Spec_fit(cubefidu.data,cubefidu.velax,rvalsf,tvalsf,vmodf,r_min,r_max,PA_min,PA_max)
    with open(dirName+'Spec_fit_rmin{:4.2f}_rmax{:4.2f}_params_fiducial.dat'.format(r_min,r_max),'w+') as f:
        f.write('Averages of a b c d\n')
        f.write('%13.6e %13.6e %13.6e %13.6e\n'%(af.mean(),bf.mean(),cf.mean(),df.mean()))
        f.write('a b c d idx idy\n')                   # Include r,theta, phi in coordinates
        for i in range(len(a)):
            f.write('%13.6e %13.6e %13.6e %13.6e %4d %4d\n'%(af[i],bf[i],cf[i],df[i],idxf[i],idyf[i]))
if double == 'T':
    af,bf,cf,af2,df,idxf,idyf = Spec_double(cubefidu.data,cubefidu.velax,rvalsf,tvalsf,vmodf,r_min,r_max,PA_min,PA_max) #b2,c2,
    with open(dirName+'Spec_fit_rmin{:4.2f}_rmax{:4.2f}_params_fiducial_double.dat'.format(r_min,r_max),'w+') as f:
        f.write('Averages of a0 b0 c0 a1 d0\n') #b1 c1
        f.write('%13.6e %13.6e %13.6e %13.6e %13.6e \n'%(af.mean(),bf.mean(),cf.mean(),af2.mean(),df.mean())) #%13.6e %13.6e # ,b2.mean(),c2.mean()
        f.write('a0 b0 c0 a1 d0 idx idy\n')                   # Include r,theta, phi in coordinates  b1 c1
        for i in range(len(a)):
            f.write('%13.6e %13.6e %13.6e %13.6e %13.6e %4d %4d\n'%(af[i],bf[i],cf[i],af2[i],df[i],idxf[i],idyf[i])) # %13.6e %13.6e #b2[i],c2[i],
"""
# =======================================================================================
# Observation
# =======================================================================================
...
for i in range(3):#len(del_x)
    ...
    spec_turb = cube_turb.data[:,iy,ix]
    spec_fidu = cubefidu.data[:,iy,ix]
    ...
    ax1.plot(x1,spec,'r',label='Ther')
    #ax1.plot(x1,shifted_spec,'r--')
    if double == 'F': ax1.plot(x1,Gaussian(x1,a[i],b[i],c[i],d[i]),'r-.',lw=0.5)
    if double == 'T': ax1.plot(x1,Double(x1,a[i],b[i],c[i],a2[i],d[i]),'r-.',lw=0.5) #b2[i],c2[i],
    ax1.plot(x1,spec_wind,'g',label='Ther+Wind')
    #ax1.plot(x1,shifted_wind,'b--')
    if double == 'F': ax1.plot(x1,Gaussian(x1,aw[i],bw[i],cw[i],dw[i]),'g-.',lw=0.5)
    if double == 'T': ax1.plot(x1,Double(x1,aw[i],bw[i],cw[i],aw2[i],dw[i]),'g-.',lw=0.5) #,bw2[i],cw2[i]
    ax1.plot(x1,spec_turb,'b',label='Ther+Turb')
    #ax1.plot(x1,shifted_wind,'b--')
    if double == 'F': ax1.plot(x1,Gaussian(x1,at[i],bt[i],ct[i],dt[i]),'b-.',lw=0.5)
    if double == 'T': ax1.plot(x1,Double(x1,at[i],bt[i],ct[i],at2[i],dt[i]),'b-.',lw=0.5) #,bw2[i],cw2[i]
    ax1.plot(x1,spec_fidu,'k',label='Ther+Turb+Wind')
    #ax1.plot(x1,shifted_wind,'b--')
    if double == 'F': ax1.plot(x1,Gaussian(x1,af[i],bf[i],cf[i],df[i]),'k-.',lw=0.5)
    if double == 'T': ax1.plot(x1,Double(x1,af[i],bf[i],cf[i],af2[i],df[i]),'k-.',lw=0.5) #,bw2[i],cw2[i]
